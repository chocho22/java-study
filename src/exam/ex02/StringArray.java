...
//	}
	public static void main(String[] args) {
		...
		// 생성자가 아예 없어서 기본생성자가 숨어있거나, 기본생성자를 위에 이미 만들어둬서.
		// ㄴ파라미터가 있는 생성자만 있을 경우에는 기본생성자를 만들어줘야 오류가 나지 않는다.
//		sa.strs = new String[5];  // 3. StringArray 생성자 메모리 만든 후 여기에 초기화 해줘도됨.
		...
		// ㄴstatic 변수는 이클립스에서 글꼴이 기울어지게 해준다.
		// scan : Resource leak. 메모리 낭비, 유출. scan.close(); 해주면 메모리 낭비가 없어짐.
		// ㄴ그러니까 사용 다 했으면 close 해주기.
		// new Scanner(System.in);  =>  (System.in) 한 이유는, Scanner의 기본생성자가 없어서.
		// Scanner scan = new Scanner(System.in); 얘만 썼을 땐 오류남. Ctrl+Shift+O 해주면
		// ㄴimport java.util.Scanner; 가 클래스 위에 뜨면서 오류 해결됨.
		// System.in  => System중에서 in, 콘솔 창과 연결.
		String input = scan.nextLine();
		System.out.println(input);
		sa.strs = input.split(",");  // , 기준으로 split해서 sa.strs의 배열에 차레대로 저장, 리턴.
		// split은 메소드. split의 데이터타입 스트링 배열. 그래서 strs에 대입할수있음.
		// String 클래스에 split이라는 메소드가 있음. 여기서 데이터타입이 스트링 배열임.
